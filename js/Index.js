...
async function getHourlyData(cameraId, channel, startDate, endDate) {
  try {
    console.log(`Fetching hourly data for camera ${cameraId} from ${startDate} to ${endDate}`);
    ...
    console.log(`Got token ${token} and totalCount ${totalCount} for camera ${cameraId}`);
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes('.StartTime=') && i + 1 < lines.length && lines[i+1].includes('.EnteredSubtotal=')) {
          ...
          if (timeMatch) {
            ...
            if (hour >= 8 && hour <= 21) {
              ...
              if (!isNaN(enteredValue)) {
                ...
                console.warn(`Invalid value for hour ${hour}: ${enteredValueStr}`);
              }
            }
          }
        }
    ...
    await fetch(`api_proxy.php?action=stopFind&camera=${cameraId}&channel=${channel}&token=${token}`);
...
function updateCameraStatus(cameraId, isOnline) {
  const statusElements = document.querySelectorAll(`.camera${cameraId}-in .status-indicator, .camera${cameraId}-out .status-indicator`);
